...
public class VistaTexto extends Vista{
	...
	public void listarAlquileresCliente() {
		...
		try {
			...
			System.out.print(
					listaAlquileresCliente.isEmpty() ? "No hay Alquileres para el cliente" : listaAlquileresCliente);
		} catch (IllegalArgumentException | NullPointerException e) {
			System.out.print(e.getMessage());
		}
	}
	...
	public void listarAlquileresVehiculo() {
		try {
			Consola.mostrarCabecera("Opcion 17 -> Listar los alquileres de un Vehículo");
			System.out.println();
			...
			System.out.print(listaAlquileresVehiculo.isEmpty() ? "No hay alquileres para el vehículo"
	...
	public void mostrarEstadisticasMensualesTipoVehiculo() {
		...
		System.out.println();
		LocalDate fechaDelMes = Consola.leerMes();

		Map<TipoVehiculo, Integer> ocurrencias = inicializarEstadisticas();
		for (Alquiler alquiler : getControlador().getAlquileres()) {
			if ((alquiler.getFechaAlquiler().getYear() == fechaDelMes.getYear())
					&& (alquiler.getFechaAlquiler().getMonth().equals(fechaDelMes.getMonth()))) {
